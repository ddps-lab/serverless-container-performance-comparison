...
resource "aws_lambda_function" "lambda" {
  function_name = "${var.prefix}-aws-lambda-${var.model_name}"
  package_type  = "Image"
...
resource "aws_apigatewayv2_api" "lambda-api" {
  name          = "${var.prefix}-aws-lambda-${var.model_name}-api"
  protocol_type = "HTTP"
}

resource "aws_apigatewayv2_stage" "lambda-api-stage" {
  api_id      = aws_apigatewayv2_api.lambda-api.id
  name        = "${var.prefix}-aws-lambda-${var.model_name}-api-stage"
  auto_deploy = true
...
resource "aws_acm_certificate" "certificate" {
  domain_name       = "${replace(var.model_name, "_", "-")}.${var.prefix}.${var.route53_domain}"
