resource "aws_iam_role" "lambda-role" {
  name = "${var.prefix}-aws-sagemaker-scpc-bench-role"
...
resource "aws_apigatewayv2_api" "lambda-api" {
  name          = "${var.prefix}-aws-sagemaker-scpc-bench-api"
  protocol_type = "HTTP"
}

resource "aws_apigatewayv2_stage" "lambda-api-stage" {
  api_id      = aws_apigatewayv2_api.lambda-api.id
  name        = "${var.prefix}-aws-sagemaker-scpc-bench-api-stage"
  auto_deploy = true
...
resource "aws_acm_certificate" "certificate" {
  domain_name       = "scpcsagemakerbench.${var.prefix}.${var.route53_domain}"
