resource "aws_iam_role" "lambda-role" {
  name = "${var.prefix}-gcp-run-rest-scpc-bench-role"
...
resource "aws_lambda_function" "lambda" {
  function_name = "${var.prefix}-gcp-run-rest-scpc-bench"
  package_type  = "Image"
...
resource "aws_apigatewayv2_api" "lambda-api" {
  name          = "${var.prefix}-gcp-run-rest-scpc-bench-api"
  protocol_type = "HTTP"
}

resource "aws_apigatewayv2_stage" "lambda-api-stage" {
  api_id      = aws_apigatewayv2_api.lambda-api.id
  name        = "${var.prefix}-gcp-run-rest-scpc-bench-api-stage"
  auto_deploy = true
...
resource "aws_acm_certificate" "certificate" {
  domain_name       = "scpcrunrestbench.${var.prefix}.${var.route53_domain}"
