...
def run_bench(num_tasks, server_address, use_https):
    ...
    request.inputs['bert_input_ids'].CopyFrom(tf.make_tensor_proto(input_ids, shape=[1, 128]))
    request.inputs['bert_input_masks'].CopyFrom(tf.make_tensor_proto(input_mask, shape=[1, 128]))
